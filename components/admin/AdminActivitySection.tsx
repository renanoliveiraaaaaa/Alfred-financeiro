...
export default function AdminActivitySection({ dailySignups, latestUsers }: Props) {
  ...
  return (
    <section className="mt-10">
      <h2 className="text-lg font-semibold text-main">{t('admin.activity.title')}</h2>
      <p className="mt-1 text-sm text-muted">{t('admin.activity.subtitle')}</p>

      <div className="mt-6 grid gap-6 lg:grid-cols-5">
        <div className="rounded-xl border border-border bg-surface p-4 shadow-sm sm:p-5 lg:col-span-3">
          <h3 className="text-sm font-medium text-main">{t('admin.activity.chartTitle')}</h3>
          <div className="mt-6 flex h-44 items-end gap-1 sm:gap-0.5">
            {dailySignups.map((day) => {
              ...
              return (
                ...
                >
                  <span className="mb-1 text-[10px] font-medium tabular-nums text-muted opacity-0 transition-opacity group-hover:opacity-100 sm:text-xs">
                    {day.count > 0 ? day.count : ''}
                  </span>
                  <div
                    className="w-full max-w-[14px] rounded-t bg-brand/80 transition-colors group-hover:bg-brand"
          ...
          <div className="mt-2 flex justify-between text-[10px] text-muted sm:text-xs">
        ...
        <div className="rounded-xl border border-border bg-surface p-4 shadow-sm sm:p-5 lg:col-span-2">
          <h3 className="text-sm font-medium text-main">{t('admin.activity.latestTitle')}</h3>
          <ul className="mt-4 space-y-3">
            {formattedUsers.map((u) => (
              <li key={u.id} className="flex items-center justify-between gap-2 border-b border-border pb-3 last:border-0 last:pb-0">
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-main">{u.name}</p>
                  <p className="text-xs text-muted">{u.when}</p>
                </div>
                <Link
                  href={`/admin/users/${u.id}`}
                  className="inline-flex min-h-[44px] shrink-0 items-center text-xs font-medium text-brand underline-offset-2 hover:underline sm:min-h-0"
                >
