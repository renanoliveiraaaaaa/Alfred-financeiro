...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="p-4 lg:p-8">
      ...
      <p className="mt-1 text-sm text-muted">{serverT('admin.dashboard.subtitle', locale)}</p>
        ...
        {kpisUsers.map(({ title, value, subtitle, Icon }) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-background text-main">
                <Icon className="h-4 w-4" strokeWidth={2} aria-hidden />
              </span>
            </div>
            <p className="mt-4 text-2xl font-semibold tabular-nums tracking-tight text-main">
              {value}
            </p>
      ...
      <h2 className="mt-10 text-base font-semibold text-main">
        {serverT('admin.dashboard.usageTitle', locale)}
      </h2>
      <p className="mt-1 text-sm text-muted">{serverT('admin.dashboard.usageSubtitle', locale)}</p>
      ...
      <p className="mt-1 text-sm text-muted">{serverT('admin.dashboard.monetizationSubtitle', locale)}</p>
      ...
      {!errLatest && !errProf30 ? (
        ...
        <p className="mt-8 text-sm text-muted">{serverT('admin.dashboard.activityLoadError', locale)}</p>
      )}
    </div>
  )
}
